...
mod tests {
    ...
    fn test_default_prompts_contain_variables() {
        let prompts = DefaultPrompts::default();
    ...
    #[test]
    fn test_hunt_prompts_all_return_content() {
        let src = "test";
        assert!(!injection_hunt_prompt(src).is_empty());
        assert!(!auth_hunt_prompt(src).is_empty());
        assert!(!xss_hunt_prompt(src).is_empty());
        assert!(!path_traversal_hunt_prompt(src).is_empty());
        assert!(!crypto_hunt_prompt(src).is_empty());
        assert!(!resource_hunt_prompt(src).is_empty());
        assert!(!deserialization_hunt_prompt(src).is_empty());
    }
    ...
    fn test_template_variables_operations() {
        ...
        assert_eq!(vars.get("K1"), Some(&"v1".to_string()));
        vars.insert("K1".to_string(), "v2".to_string());
        assert_eq!(vars.get("K1"), Some(&"v2".to_string()));
    }
}
