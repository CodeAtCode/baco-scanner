...
pub fn analyze_diff(input: DiffAnalysisInput) -> Result<DiffAnalysisOutput, Box<dyn std::error::Error>> {
...
fn run_diff(file_path: &str, base: Option<&str>, head: &str) -> Result<DiffAnalysisOutput, Box<dyn std::error::Error>> {
    let base_str = base.map(|s| format!("{}..{}", s, head)).unwrap_or_else(|| head.to_string());
    
    let output = Command::new("git")
        .args(["diff", &base_str, "--", file_path])
        .current_dir(PathBuf::from(file_path).parent().unwrap_or(&PathBuf::from(".")))
        .output()
