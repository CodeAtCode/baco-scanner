...
"#;
        ...
        let context = TriplePathContext::build(source, Language::C, &kb, 3)
            .expect("Should build triple path");

        assert!(!context.control.ast_text.is_empty());
        // Knowledge path may be empty if no matches found - make optional
    ...
    fn test_to_prompt_section_format() {
        ...
        let context = TriplePathContext::build(source, Language::Rust, &kb, 2).unwrap();
        let context = context.with_semantic("Test function summary".to_string());
        ...
        assert!(prompt.contains("Test function summary"));
    }

    #[test]
    fn test_prompt_section_non_empty() {
        let source = "int main() { return 0; }";
        let kb = CweKnowledgeBase::load_embedded().unwrap();
        let context = TriplePathContext::build(source, Language::C, &kb, 1).unwrap();

        let prompt = context.to_prompt_section();
        assert!(!prompt.is_empty(), "Prompt section should not be empty");
        assert!(
            prompt.len() > 100,
            "Prompt section should have substantial content"
        );
    }
}
