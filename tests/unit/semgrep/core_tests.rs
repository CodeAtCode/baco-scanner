//! Inline tests migrated from src/semgrep/mod.rs (Wave 3)
//!
//! These tests were extracted from the inline #[cfg(test)] block to comply
//! with project rule #5775: NO inline test blocks in src/
...
fn test_extract_code_snippet_file_not_found() {
    let snippet = baco::semgrep::parser::extract_code_snippet("/nonexistent/file.rs", 42, 2);
...
fn test_extract_code_snippet_with_context() {
    ...
    let snippet = baco::semgrep::parser::extract_code_snippet(test_file.to_str().unwrap(), 3, 1);
...
fn test_extract_code_snippet_target_line_at_start() {
    ...
    let snippet = baco::semgrep::parser::extract_code_snippet(test_file.to_str().unwrap(), 1, 2);
...
fn test_extract_code_snippet_target_line_beyond_file() {
    ...
    let snippet = baco::semgrep::parser::extract_code_snippet(test_file.to_str().unwrap(), 100, 2);
...
fn test_extract_code_snippet_file_read_error() {
    ...
    let temp_dir = std::env::temp_dir();
    let snippet = baco::semgrep::parser::extract_code_snippet(temp_dir.to_str().unwrap(), 1, 2);
...
fn test_extract_code_snippet_fewer_lines_than_context() {
    ...
    let snippet = baco::semgrep::parser::extract_code_snippet(test_file.to_str().unwrap(), 1, 5);
...
fn test_extract_code_snippet_exact_context_fit() {
    ...
    let snippet = baco::semgrep::parser::extract_code_snippet(test_file.to_str().unwrap(), 3, 2);
...
fn test_extract_code_snippet_file_with_single_line() {
    ...
    let snippet = baco::semgrep::parser::extract_code_snippet(test_file.to_str().unwrap(), 5, 2);
...
fn test_semgrep_runner_new_with_multiple_exclude_rules() {
    let runner = SemgrepRunner::new(vec![], vec!["rule1".into(), "rule2".into(), "rule3".into()]);
...
fn test_extract_code_snippet_line_zero() {
    ...
    let snippet = baco::semgrep::parser::extract_code_snippet(test_file.to_str().unwrap(), 0, 1);
    assert!(snippet.contains("line 1"));
...
fn test_extract_code_snippet_large_context() {
    ...
    let snippet = baco::semgrep::parser::extract_code_snippet(test_file.to_str().unwrap(), 2, 100);
    assert!(snippet.contains("line 1"));
    assert!(snippet.contains("line 2"));
    assert!(snippet.contains("line 3"));
...
fn test_extract_code_snippet_target_at_start() {
    ...
    let snippet = baco::semgrep::parser::extract_code_snippet(test_file.to_str().unwrap(), 1, 1);
    assert!(snippet.contains("line 1"));
    assert!(snippet.contains("line 2"));
...
fn test_raw_finding_struct() {
    use baco::semgrep::rules::RawFinding;
    let raw = RawFinding {
        path: "test.rs".into(),
        line: 42,
        end_line: 45,
...
fn test_severity_mapping_all_variants() {
    use baco::semgrep::rules::parse_severity;
