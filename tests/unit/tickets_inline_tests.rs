...
// Disabled: references private item `extract_meaningful_words` (pub(super) in search.rs, not accessible from external tests)
/*
#[test]
fn test_extract_meaningful_words_basic() {
    let result = search::extract_meaningful_words("vulnerability in authentication", 2);
...
fn test_extract_meaningful_words_excludes_stop_words() {
    let result = search::extract_meaningful_words("the quick brown fox jumps", 4);
...
fn test_extract_meaningful_words_excludes_short_words() {
    let result = search::extract_meaningful_words("a an to of in on", 5);
...
fn test_extract_meaningful_words_case_insensitive() {
    let result = search::extract_meaningful_words("THE Quick Brown", 2);
    assert!(!result.contains("THE"));
    assert!(result.contains("Quick"));
    assert!(result.contains("Brown"));
}
...
fn test_extract_meaningful_words_empty_string() {
    let result = search::extract_meaningful_words("", 5);
    assert!(result.is_empty());
}
*/
...
fn test_extract_meaningful_words_limit() {
    let result = search::extract_meaningful_words("one two three four five", 3);
...
fn test_extract_meaningful_words_excludes_no_missing_lack() {
    let result = search::extract_meaningful_words("no missing lack of credentials", 3);
...
// Disabled: references private item `parse_github_url` (pub(crate) in search.rs, not accessible from external tests)
/*
#[tokio::test]
...
// Disabled: references private field `TicketSearcher.systems` (pub(crate), not accessible from external tests)
/*
#[tokio::test]
...
fn test_extract_meaningful_words_single_word() {
    let result = search::extract_meaningful_words("vulnerability", 5);
...
fn test_extract_meaningful_words_all_stop_words() {
    let result = search::extract_meaningful_words("the a an is are", 5);
    assert!(result.is_empty());
}
*/
...
fn test_extract_meaningful_words_exact_count() {
    let result = search::extract_meaningful_words("one two three", 2);
    let words: Vec<&str> = result.split_whitespace().collect();
...
fn test_extract_meaningful_words_zero_count() {
    let result = search::extract_meaningful_words("test words here", 0);
    assert!(result.is_empty());
}
*/
...
fn test_extract_meaningful_words_large_count() {
    let result = search::extract_meaningful_words(
