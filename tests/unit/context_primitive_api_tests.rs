...
fn test_vuln_type_null_pointer_deref() {
    assert_eq!(PrimitiveApiVulnType::NullPointerDeref.as_str(), "NullPointerDeref");
}
...
fn test_table_unique_names() {
    ...
    assert_eq!(names.len(), unique_names.len(), "All entries should have unique names");
}
...
fn test_table_total_vuln_coverage() {
    ...
    for vuln in all_vulns {
        let found = PRIMITIVE_API_TABLE.iter().any(|e| e.vuln_types.contains(&vuln));
        assert!(found, "Vuln type {:?} should be covered by at least one API", vuln);
    }
}
...
fn test_entry_debug_format() {
    let entry = PRIMITIVE_API_TABLE.iter().find(|e| e.name == "free").unwrap();
...
#[test]
fn test_entry_clone() {
    let entry = PRIMITIVE_API_TABLE.iter().find(|e| e.name == "malloc").unwrap();
...
fn test_lookup_all_table_entries() {
    for entry in PRIMITIVE_API_TABLE {
        ...
        assert_eq!(vulns, entry.vuln_types, "Lookup should return same vuln types as table");
    }
}
...
fn test_resource_leak_only_for_io_apis() {
    let io_apis = ["open", "socket", "fopen", "fdopen", "opendir", "close", "fclose", "closedir"];
...
fn test_memory_apis_have_multiple_vuln_types() {
    ...
    assert!(malloc_vulns.len() > 1, "malloc should have multiple vuln types");
