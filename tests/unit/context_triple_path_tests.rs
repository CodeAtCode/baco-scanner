...
use baco::context::triple_path::{TriplePathContext, ContextError};
...
fn test_with_semantic_adds_summary() {
    ...
    assert_eq!(context_with_semantic.semantic_summary.unwrap(), "Test summary");
}
...
fn test_with_semantic_preserves_other_fields() {
    ...
    assert_eq!(context_with_semantic.knowledge.retrieved_rules.len(), knowledge_before);
}
