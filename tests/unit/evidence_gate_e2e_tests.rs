...
fn test_json_gate_on_empty_findings_valid_empty_array() {
    ...
    write_findings_json(&[], &[], &path, None, Some(&gate_config()), None, None).unwrap();
...
fn json_gate_on_includes_all_findings_with_tiers() {
    ...
    let cfg = gate_config();
    write_findings_json(&findings, &[], path, None, Some(&cfg), None, None).unwrap();
