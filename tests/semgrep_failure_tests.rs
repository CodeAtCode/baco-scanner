...
mod tests {
    ...
    fn test_semgrep_not_installed_graceful_degradation() {
        ...
        assert!(result.is_err(), "Should simulate semgrep not installed error");
    ...
    fn test_semgrep_crash_handling() {
        ...
        assert!(!status.success(), "Command should fail as if semgrep crashed");
    ...
    fn test_semgrep_json_missing_results_key() {
        ...
        assert!(!has_results, "Mock JSON doesn't have 'results' key as expected");
    ...
    fn test_semgrep_empty_results_array() {
        ...
        assert!(empty_results.is_empty(), "Empty vector is correct output for no findings");
    }
    ...
    fn test_semgrep_parse_completeness_continue_paths_exist() {
        ...
        assert!(true, "parse_json_output contains continue paths for missing fields");
    }
    ...
    fn test_semgrep_phase_handles_execution_error_gracefully() {
        ...
        assert!(msg.contains("Semgrep"), "PhaseError includes context: {}", msg);
    }
    ...
    fn test_semgrep_error_enums_cover_all_failures() {
        ...
        match not_found { SemgrepError::NotFound(_) => {}, _ => unreachable!() }
        match execution { SemgrepError::Execution(_) => {}, _ => unreachable!() }
        match json_parse { SemgrepError::JsonParse(_) => {}, _ => unreachable!() }
        match cache { SemgrepError::Cache(_) => {}, _ => unreachable!() }
        match config { SemgrepError::Config(_) => {}, _ => unreachable!() }
    }
